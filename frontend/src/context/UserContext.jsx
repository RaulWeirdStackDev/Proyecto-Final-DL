...
const UserProvider = ({ children }) => {
    ...
    const login = async (email, password) => {
        try {
            const response = await fetch('https://pizzeriamammamia.onrender.com/api/auth/login', {  // URL actualizada
                method: 'POST',
    ...
    const register = async (email, password) => {
        try {
            const response = await fetch('https://pizzeriamammamia.onrender.com/api/auth/register', {  // URL actualizada
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email, password }),
            });
